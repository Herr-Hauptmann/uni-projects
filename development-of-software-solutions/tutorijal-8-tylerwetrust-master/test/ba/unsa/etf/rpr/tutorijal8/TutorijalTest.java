...
class TutorijalTest {
    ...
    void daLiUcitavaGradoveKorektno() {
        ...
        try {
            ...
            assertAll(()->
            {
                ...
                assertEquals(9+1, Arrays.stream(gradovi.get(0).getTemperature()).distinct().count());
                assertEquals("Split",gradovi.get(1).getNaziv());
                assertEquals(4+1, Arrays.stream(gradovi.get(1).getTemperature()).distinct().count());
            });
    ...
    void daLiSeNeKrahiraPriUcitavanju2000() {
        ...
        try{
            ...
            printWriter.close();
            assertDoesNotThrow((org.junit.jupiter.api.function.Executable) Tutorijal::ucitajGradove);
        } catch (FileNotFoundException e) {
            e.printStackTrace();
        } catch (UnsupportedEncodingException e) {
            e.printStackTrace();
